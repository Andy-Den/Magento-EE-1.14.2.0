...
Mage::log('Go to Datafeed', null, 'datafeed.log');
...
if(trim($secureHashSecret) != ""){	
	...
	while ( list ( $key, $value ) = each ( $secureHashs ) ) {
		$verifyResult = verifyPaymentDatafeed($src, $prc, $successCode, $ref, $payRef, $cur, $amt, $payerAuth, $secureHashSecret, $value);
		if ($verifyResult) {
			break ;
		}
	}	
...
Mage::log('My log entry', null, 'datafeed.log');
Mage::log('$successCode : '.$successCode, null, 'datafeed.log');
Mage::log('$dbAmount: '.$dbAmount, null, 'datafeed.log');
Mage::log('Amount: '.$amt, null, 'datafeed.log');
Mage::log('$dbCurrencyIso : '.$dbCurrencyIso.' Cur : '.$cur, null, 'datafeed.log');
...
//			$payment->addTransaction(Mage_Sales_Model_Order_Payment_Transaction::TYPE_PAYMENT);
	...
	if($dbState == Mage_Sales_Model_Order::STATE_PROCESSING || $dbState == Mage_Sales_Model_Order::STATE_COMPLETE){
		//do nothing here
		echo "The order state is already set to  \"".dbState."\", so we cannot set it to \"".Mage_Sales_Model_Order::STATE_CANCELED."\" anymore";
	}else{
