    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        if ($putOrderId) {
            ...
            $barCodeOption = array(
                ...
                'font' => "./lib/Code39Font/IDAutomationHC39M.ttf",
                /*'barWidth' => $width,
                'barHeight' => $height,
                'drawText' => false*/
                'barHeight'=> 40,
            ...
            // POSITION FOR 9 NUMBERS
//            $renderOption = array(
//                'leftOffset' => 477,
//                'topOffset' => 27
//            );

            // POSITION FOR 5 NUMBERS
            $renderOption = array(
                /*'leftOffset' => 509,*/
                'leftOffset' => 450,
                'topOffset' => 27
            );
            Zend_Barcode::factory('code39', 'pdf', $barCodeOption, $renderOption)->setResource($this->_pdf)->draw();
        ...
        if ($order->getIsVirtual()) {
            ...
            if($order->getCustomerNote()) {
                ...
                $this->_setFontBold($page, 12);
                $page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
                $page->drawText(Mage::helper('sales')->__('Customer Comments'), 35, $this->y - 15, 'UTF-8');

                $this->y -= 35;

                $this->_setFontRegular($page, 12);
                $orderComment = $order->getCustomerComment() ? $order->getCustomerComment() : $order->getCustomerNote();
                strip_tags(trim($orderComment));
                $text = array();
                foreach (Mage::helper('core/string')->str_split($orderComment, 105, true, true) as $_value) {
                    $text[] = $_value;
                }
                foreach ($text as $part) {
                    $page->drawText($part, 35, $this->y - 10, 'UTF-8');
                    $this->y -= 15;
                }


                /*$orderComment2 = preg_replace('/<br[^>]*>/i', "\n", $orderComment);
                $page->drawText(strip_tags(trim($orderComment2)), 35, $this->y-10, 'UTF-8');*/


                $this->y -= 15;

                // replacement of Shipments-Payments rectangle block
                $page->drawLine(25, $this->y, 25, $currentY - 15); //left
                $page->drawLine(25, $this->y, 570, $this->y); //bottom
                $page->drawLine(570, $this->y, 570, $currentY - 15); //right
