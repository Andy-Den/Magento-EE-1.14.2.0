 ...
 * Last Modified: 2014-12-01T23:00:36+01:00
    ...
    private function _getUpdatedFields($updateData, $productId)
    {
        ...
        foreach ($updateData as $field => $newValue) {
            foreach ($stockItem as $stockField => $stockValue) {
                ...
                if ($field == $stockField) {
                    ...
                    /*if ($stockField == 'qty') {
                        // In order for the 5000/0 modification to work, the qty float typecasting in Xtento_StockImport_Model_Processor_Mapping_Fields::formatField must be disabled
                        if (!is_numeric($newValue)) {
                            $newValue = trim(strtolower($newValue));
                            if ($newValue === 'yes' || $newValue === 'ja' || $newValue === 'in stock' || $newValue === 'available' || $newValue === 'y' || $newValue === 'j') {
                                $newValue = 5000;
                            }
                            if ($newValue === 'no' || $newValue === 'nein' || $newValue === 'out of stock' || $newValue === 'oos' || $newValue === 'n') {
                                $newValue = 0;
                            }
                        }
                        #$stockValue = (float)preg_replace("/[^0-9.,+-]/", "", $stockValue); // Float value coming from the database
                    }*/
                    ...
                    /*if ($stockField == 'is_in_stock' && $newValue !== '') {
                        if (!is_numeric($newValue)) {
                            $newValue = trim(strtolower($newValue));
                        }
                        // Detect "In Stock" value using this criteria: (is_in_stock in database only accepts 0 or 1)
                        if ($newValue === 'true' || $newValue === 'yes' || $newValue === 'ja' || $newValue === 'in stock' || $newValue === 'available' || (is_numeric($newValue) && $newValue > 0)
                        ) {
                            $newValue = 1;
                        } else {
                            $newValue = 0;
        ...
        foreach ($updatedFields as $field => $value) {
            if ($field == 'qty' && !isset($updateData['is_in_stock'])) {
                ...
                if ((int)$stockItem['use_config_min_qty'] === 1) {
