...
class Amasty_Rules_Model_Observer
{
    ...
    protected function _initRule($rule, $address, $quote) 
    {
        ...
        $qty = $this->_getQty($rule, count($prices), $quote->getCustomerId());
        if ($qty < 1){
            return $r;
        }
        ...
        $expTypes = array(Amasty_Rules_Helper_Data::TYPE_EXPENCIVE, Amasty_Rules_Helper_Data::TYPE_EXPENCIVE_MONTH);
        if ($rule->getSimpleAction() == Amasty_Rules_Helper_Data::TYPE_CHEAPEST){
            $prices = array_slice($prices, 0, $qty);
        } 
        elseif (in_array($rule->getSimpleAction(), $expTypes)) {
     ...
     * @return int qty
     */
    protected function _getQty($rule, $cartQty, $customerId)
    {
        $discountQty    = 1;
		...
		$count = 0;
		if ($rule->getSimpleAction() == Amasty_Rules_Helper_Data::TYPE_EXPENCIVE_MONTH){
            ...
            $order_collection = Mage::getModel('sales/order')
				...
				->addAttributeToFilter('status', array('nin' => array('canceled','closed','canceled_paypalcw','unsuccessful_payment')))
			...
			if (count($order_collection) > 0) {
				...
				foreach ($order_collection as $order) {
					foreach ($order->getItemsCollection() as $item) {
						$myArray = explode(',', $item->getAppliedRuleIds());
						
						if((in_array($ruleId, $myArray)) && ($item->getDiscountAmount() > 0)){
							// This can go wrong if $maxDiscountQty is increased later 	
							$count += ((int)$item->getQtyOrdered());
							if ($count >= $maxDiscountQty) {
								return 0;
							}
						}
					}
        ...
        return min($discountQty, ($maxDiscountQty-$count));        
    }
