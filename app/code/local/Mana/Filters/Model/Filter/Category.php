    ...
    implements Mana_Filters_Interface_Filter
{
    ...
    private function _getCategoryData($category){
        $data = array();
        $sub_categories = $category->getChildrenCategories();
        if(count($sub_categories) > 0){
            // Add count to subcategories
            $this->addCountToCategories($sub_categories);
            foreach($sub_categories as $sub_category){
                $value = $this->_getCategoryData($sub_category);
                if($value !== false){
    ...
    protected function _getCategoryItemsData($categories)
    {
        ...
        foreach ($categories as $category) {
            $value = $this->_getCategoryData($category);
            if($value !== false){
                $data = array_merge($data,$value);
            }
        }
