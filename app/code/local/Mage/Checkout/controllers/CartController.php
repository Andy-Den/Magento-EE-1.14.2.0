...
class Mage_Checkout_CartController extends Mage_Core_Controller_Front_Action
{
    ...
    protected function _updateShoppingCart()
    {
        try {
            ...
            $cartData_Qty = $this->getRequest()->getParam('cart_qty');
            if (is_array($cartData)) {
                ...
                foreach ($cartData as $index => $data) {
                    $detail = explode(',', $cartData_Qty[$index]['qty']);
                    $product = Mage::getModel('catalog/product')->load($detail[0]);
                    $qty = $product->getStockItem()->getQty();

                    if ($qty >= $cartData[$index]['qty']) {
                        $cartData[$index]['qty'] = $filter->filter(trim($cartData[$index]['qty']));
                    }else{
                        $message = 'The requested quantity for "'.$detail[2].'" is not available.';
                        $this->_getSession()->addError(Mage::helper('core')->escapeHtml($message));
                        $cartData[$index]['qty'] = $filter->filter(trim($detail[1]));
                    }
                }
                $cart = $this->_getCart();
