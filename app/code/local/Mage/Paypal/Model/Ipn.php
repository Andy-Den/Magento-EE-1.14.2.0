...
class Mage_Paypal_Model_Ipn
{
    ...
    protected function _postBack(Zend_Http_Client_Adapter_Interface $httpAdapter)
    {
        ...
        $httpAdapter->write(
            ...
            $postbackUrl,
            '1.1',
          //  array('Connection: close'), // native Magento code
            array('Connection: close', 'User-Agent: Mozilla/5.0 (Windows NT 6.1; WOW64; rv:38.0) Gecko/20100101 Firefox/38.0'), // new code, fixing Paypal issue when post back
            $postbackQuery
        );
...
//            $comment->save();
    ...
    protected function _registerRecurringProfilePaymentCapture()
    {
        ...
        if ($invoice) {
            ...
            //$order->queueNewOrderEmail()->addStatusHistoryComment($message)->setIsCustomerNotified(true)->save(); // Native Magento code
            $order->addStatusHistoryComment($message)->setIsCustomerNotified(true)->save(); //  Updating code to remove sending order email
        }
    }
    ...
    protected function _registerPaymentCapture($skipFraudDetection = false)
    {
        ...
        if ($invoice && !$this->_order->getEmailSent()) {
            //$this->_order->queueNewOrderEmail()->addStatusHistoryComment // Native Magento code
            $this->_order->addStatusHistoryComment // Updating code to remove sending order email
            (
    ...
    protected function _registerPaymentAuthorization()
    {
        ...
        if (!$this->_order->getEmailSent()) {
          //  $this->_order->queueNewOrderEmail(); //Remove sending order email
        }
        $this->_order->save();
    }
